...
Images can be built locally using the following commands:
...
docker buildx build --push \
  ...
  --tag us-docker.pkg.dev/qa-wolf/third-party/gke-release/gcs-fuse-csi-driver-webhook:TAG \
  --platform linux/amd64 .
```

Build the CSI driver image:

```
docker buildx build --push \
  --file ./cmd/csi_driver/Dockerfile \
  --tag us-docker.pkg.dev/qa-wolf/third-party/gke-release/gcs-fuse-csi-driver:TAG \
  --platform linux/amd64 .
```
